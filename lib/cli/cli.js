...
const logger = require('heimdalljs-logger')('leanes-cli:cli');
const loggerTesting = require('heimdalljs-logger')('leanes-cli:testing');
  ...
  async run(environmentPromiseHash) {
    ...
    try {
      ...
      getOptionArgs('--verbose', commandArgs).forEach((arg) => {
        process.env[`LEANES_VERBOSE_${arg.toUpperCase()}`] = 'true';
      });
      ...
      if (!this.testing) {
        if (platform.isDeprecated) {
          this.ui.writeDeprecateLine(`Node ${process.version} is no longer supported by LeanES CLI.${recommendation}`);
        }

        if (!platform.isTested) {
          this.ui.writeWarnLine(
            `Node ${process.version} is not tested against LeanES CLI on your platform.${recommendation}`
          );
        }
      }
      ...
      if (!this.testing) {
        ...
        if (environment.project.isLeanesCLIProject() && !skipInstallationCheck) {
