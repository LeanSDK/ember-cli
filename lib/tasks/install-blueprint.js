...
const logger = require('heimdalljs-logger')('leanes-cli:tasks:install-blueprint');
...
class InstallBlueprintTask extends Task {
  ...
  async _createEmptyPackageJSON(tempProjectPath, blueprintName) {
    return fs.writeFile(
      ...
      JSON.stringify({
        name: 'leanes-cli-blueprint-install-wrapper',
        version: '0.0.0',
  ...
  async _createPackageJSON(tempProjectPath, descriptor) {
    let packageName = descriptor.name || `leanes-cli-blueprint-${Date.now()}`;
  ...
  _validateNpmModule(modulePath, packageName) {
    logger.info(`Checking for "leanes-blueprint" keyword in "${packageName}" module ...`);
    let pkg = require(this._resolvePackageJSON(modulePath));
    if (!pkg || !pkg.keywords || pkg.keywords.indexOf('leanes-blueprint') === -1) {
      throw new SilentError(`The package '${packageName}' is not a valid LeanES CLI blueprint.`);
    }
  }
   ...
   * such as `leanes init -b @company/company-specific-blueprint` where the module is in a different
   * registry.
   */
  _copyNpmrc(tmpDir) {
