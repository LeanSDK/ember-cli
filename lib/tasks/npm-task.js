...
const logger = require('heimdalljs-logger')('leanes-cli:npm-task');
    ...
    try {
      ...
      if (error.code === 'ENOENT') {
        throw new SilentError(
          'LeanES CLI is now using yarn, but was not able to find it.\n' +
  ...
  async checkNpmVersion() {
    try {
      ...
      if (!ok) {
        ...
        if (below) {
          throw new SilentError(
            'LeanES CLI is now using the global npm, but your npm version is outdated.\n' +
              'Please update your global npm version by running: npm install -g npm'
          );
        }

        this.ui.writeWarnLine(
          'LeanES CLI is using the global npm, but your npm version has not yet been ' +
            'verified to work with the current LeanES CLI release.'
        );
      }
      ...
      if (error.code === 'ENOENT') {
        throw new SilentError(
          'LeanES CLI is now using the global npm, but was not able to find it.\n' +
