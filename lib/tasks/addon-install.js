...
class AddonInstallTask extends Task {
          ...
          if (blueprintName) {
            ...
            addonInstall.ui.writeWarnLine(
              `Could not figure out blueprint name from: "${packageName}". ` +
                `Please install the addon blueprint via "leanes generate <addon-name>" if necessary.`
            );
          }
        }),
      Promise.resolve()
  ...
  findDefaultBlueprintName(givenName) {
    const packageName = getPackageBaseName(givenName);
    if (!packageName) {
      return null;
    }

    const addon = this.project.findAddonByName(packageName);
    if (!addon) {
      throw new SilentError(`Install failed. Could not find addon with name: ${givenName}`);
    }

    const leanesAddon = addon.pkg['leanes-addon'];

    if (leanesAddon && leanesAddon.defaultBlueprint) {
      return leanesAddon.defaultBlueprint;
    }
