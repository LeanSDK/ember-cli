...
in leanes-cli, i.e. `leanes (new||init||addon) app-name --lang=langArg`
...
language. For example, the user might call `leanes new my-app --lang=typescript`
expecting to achieve an effect similar to the one provided by the
`leanes-cli-typescript` addon.
...
-------------------------------------------------------------------------------
This is a workaround for handling an existing bug in the leanes-cli parser
...
As examples, the parser behavior would likely affect the following usages:
  1. `leanes new app-name --lang --skip-npm
  2. `leanes new app-name --lang`
...
In the above examples, `langArg` would likely be assigned as follows:
  1. `leanes new app-name --lang --skip-npm => `langArg='--skip-npm'`
  2. `leanes new app-name --lang` => `langArg='--disable-analytics'`
...
Information about using the \`--lang\` flag:
  ...
  See \`leanes <command> help\` for more information.`;
