 ...
 *
 * @method contentFor
 * @param {Object} config LeanES application configuration
...
function contentFor(config, match, type, options) {
  ...
  // This normalizes `rootURL` to the value which we use everywhere inside of LeanES CLI.
  ...
  switch (type) {
    ...
    case 'test-body-footer':
      content.push(
        `<script>LeanES.assert('The tests file was not loaded. Make sure your tests index.html includes "assets/tests.js".', LeanesENV.TESTS_FILE_LOADED);</script>`
      );
...
function configReplacePatterns(options) {
  return [
    ...
    {
      match: /{{\s?LEANES_ENV\s?}}/g,
      replacement(config) {
        return convertObjectToString(config.LeanesENV);
      },
    },
    {
      match: /{{content-for ['"](.+?)["']}}/g,
