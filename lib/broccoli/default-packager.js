...
const leanesAppUtils = require('../utilities/leanes-app-utils');
...
const LEANES_CLI_INTERNAL_FILES_PATH = '/vendor/ember-cli/';
const LEANES_CLI_FILES = [
  'app-boot.js',
  'app-config.js',
  'app-prefix.js',
  'app-suffix.js',
...
const configReplacePatterns = leanesAppUtils.configReplacePatterns;
...
/**
 * Responsible for packaging LeanES application.
 *
...
module.exports = class DefaultPackager {
  constructor(options) {
    ...
    this._cachedleanesCLIInternalTree = null;
  ...
  processAppAndDependencies(allTrees) {
    if (this._cachedProcessedAppAndDependencies === null) {
      const config = this.packageConfig();
      const internal = this.packageleanesCLIInternalFiles();
      const appContentsWithCompiledTemplates = this._debugTree(
        this.processTemplates(allTrees),
        'app-and-deps:post-templates'
      );

      const trees = [allTrees, appContentsWithCompiledTemplates].filter(Boolean);

      const mergedTree = this._debugTree(
        mergeTrees(trees, {
      ...
      const external = this.applyCustomTransforms(allTrees);
      const postprocessedApp = this.processJavascript(mergedTree);

      const sourceTrees = [external, postprocessedApp, config, internal];
  ...
  /*
   * Returns a single tree with `leanes-cli` internal files with the following
   * structure:
   *
   * ```
   * vendor/
   ...
   * internal `leanes-cli` variables, such as:
   *
   * + `{{MODULE_PREFIX}}`
   * + different types of `{{content-for}}` (`{{content-for 'app-boot'}}`)
   *
   * @private
   * @method packageleanesCLIInternalFiles
   * @return {BroccoliTree}
   */
  packageleanesCLIInternalFiles() {
    if (this._cachedleanesCLIInternalTree === null) {
      let patterns = configReplacePatterns({
        addons: this.project.addons,
        autoRun: this.autoRun,
        storeConfigInMeta: this.storeConfigInMeta,
      ...
      let leanesCLITree = new ConfigReplace(new UnwatchedDir(__dirname), configTree, {
        configPath,
        files: LEANES_CLI_FILES,
        patterns,
      });

      this._cachedleanesCLIInternalTree = new Funnel(leanesCLITree, {
        files: LEANES_CLI_FILES,
        destDir: LEANES_CLI_INTERNAL_FILES_PATH,
        annotation: 'Packaged LeanES CLI Internal Files',
      });
    }

    return this._cachedleanesCLIInternalTree;
  }
  ...
  packageTestApplicationConfig() {
    ...
    let configPath = path.join(this.name, 'config', 'environments', `test.json`);
    let leanesCLITree = new ConfigReplace(new UnwatchedDir(__dirname), this.packageConfig(), {
      configPath,
      files,
      patterns,
    });

    return new Funnel(leanesCLITree, {
      files,
      srcDir: '/',
  ...
  packageApplicationTests(tree) {
    let appTestTrees = []
      .concat(this.packageleanesCLIInternalFiles(), this.packageTestApplicationConfig(), tree)
      .filter(Boolean);
  ...
  packageTestFiles(tree, coreTestTree) {
    ...
    let leanesCLITree = this.packageleanesCLIInternalFiles();
    ...
    let baseMergedTree = mergeTrees([leanesCLITree, tree, external, coreTestTree], {
      overwrite: true,
    });
