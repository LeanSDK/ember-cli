...
const logger = require('heimdalljs-logger')('leanes-cli:leanes-app');
const addonProcessTree = require('../utilities/addon-process-tree');
const lintAddonsByType = require('../utilities/lint-addons-by-type');
const leanesCLIBabelConfigKey = require('../utilities/leanes-cli-babel-config-key');
...
class LeanesApp {
  /**
   LeanesApp is the main class LeanES CLI uses to manage the Broccoli trees
  ...
  constructor(defaults, options) {
    ...
    this._debugTree = BroccoliDebug.buildDebugCallback('leanes-app');
  ...
  /**
    ...
    Defaults to `false` unless `leanes test` was used or this is *not* a production build.
  ...
  */
  _initTestsAndHinting(options) {
    let testsEnabledDefault = process.env.LEANES_CLI_TEST_COMMAND === 'true' || !this.isProduction;
  ...
  /**
    ...
    closest LeanES CLI project from the current working directory.
  ...
  */
  ...
  /**
    ...
    a set of default values from LeanES CLI.
  ...
  */
  _initOptions(options) {
    ...
    let leanesCLIBabelInstance = this.project.findAddonByName('ember-cli-babel');
    if (leanesCLIBabelInstance) {
      ...
      let leanesCLIBabelConfigKey = this._leanesCLIBabelConfigKey();
      detectedDefaultOptions[leanesCLIBabelConfigKey] = detectedDefaultOptions[leanesCLIBabelConfigKey] || {};
      detectedDefaultOptions[leanesCLIBabelConfigKey].compileModules = true;
    }
  ...
  _leanesCLIBabelConfigKey() {
    let leanesCLIBabelInstance = this.project.findAddonByName('ember-cli-babel');

    return leanesCLIBabelConfigKey(leanesCLIBabelInstance);
  }
  ...
  */
  _initVendorFiles() {
    const bowerDeps = this.project.bowerDependencies();
    const leanes = this.project.findAddonByName('@leansdk/leanes');
    const addonleanesCLIShims = this.project.findAddonByName('ember-cli-shims');
    const bowerleanesCLIShims = bowerDeps['ember-cli-shims'];

    const developmentLeanes = leanes.paths.debug;
    const productionLeanes = leanes.paths.prod;
    const leanesTesting = leanes.paths.testing;
    const leanesShims = leanes.paths.shims;
      ...
      merge(
        {
          ...
          'ember.js': {
            development: developmentLeanes,
            production: productionLeanes,
          },
          'ember-testing.js': [leanesTesting, { type: 'test' }],
          'app-shims.js': leanesShims,
          'ember-resolver.js': [
    ...
    // certain versions of `@leansdk/leanes` bundle them by default,
    // so we must check if that is the load mechanism of leanes
    // before checking `bower`.
    let leanesCLIShimsRequired = this._checkleanesCLIBabel(this.project.addons);
    if (!leanesShims && !addonleanesCLIShims && !bowerleanesCLIShims && leanesCLIShimsRequired) {
   ...
   */
  static env() {
    return process.env.LEANES_ENV || 'development';
  }
  ...
  */
  ...
  /**
    @private
    @method _checkleanesCLIBabel
    @param {Addons} addons
    @return {Boolean}
  */
  _checkleanesCLIBabel(addons, result, roots) {
    addons = addons || [];
    result = result || false;
    roots = roots || {};
    ...
    return addons.some((addon) => this._checkleanesCLIBabel(addon.addons, result, roots)) || result;
  }
  ...
  */
  ...
  _precompileAppJsTree(tree) {
    ...
    let original = this.options[leanesCLIBabelConfigKey];

    // the app will handle transpilation after it tree-shakes
    // do it here instead of the constructor because
    // leanes-data and others do their own compilation in their
    ...
    this.options[leanesCLIBabelConfigKey] = Object.assign({}, original, {
      compileModules: false,
      disablePresetEnv: true,
      disableDebugTooling: true,
      disableLeanesModulesAPIPolyfill: true,
    });
    ...
    this.options[leanesCLIBabelConfigKey] = original;
  ...
  */
  ...
  /*
   * Gather all dependencies external to `leanes-cli`, namely:
   *
   * + app `vendor` files
