...
const logger = require('heimdalljs-logger')('leanes-cli:addons-factory');
...
function instantiateAddons(parent, project, addonPackages) {
  ...
  let addonInfo, leanesAddonConfig;

  addonNames.forEach((name) => {
    addonInfo = addonPackages[name];
    leanesAddonConfig = addonInfo.pkg['leanes-addon'];

    graph.add(name, addonInfo, leanesAddonConfig.before, leanesAddonConfig.after);
  });
