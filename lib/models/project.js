...
const logger = require('heimdalljs-logger')('leanes-cli:project');
...
class Project {
  ...
  */
  constructor(root, pkg, ui, cli) {
    ...
    */
    ...
    this.leanesCLIVersion = leanesCLIVersion;
  ...
  static nullProject(ui, cli) {
    ...
    NULL_PROJECT.isLeanesCLIProject = function () {
      return false;
    };

    NULL_PROJECT.isLeanesCLIAddon = function () {
      return false;
    };
  ...
  /**
    Returns whether or not this is an LeanES CLI project.
    This checks whether leanes-cli is listed in devDependencies.

    @private
    @method isLeanesCLIProject
    @return {Boolean} Whether this is an LeanES CLI project
   */
  isLeanesCLIProject() {
    return (this.cli ? this.cli.npmPackage : 'leanes-cli') in this.dependencies();
  }

  /**
    Returns whether or not this is an LeanES CLI addon.

    @method isLeanesCLIAddon
    @return {Boolean} Whether or not this is an LeanES CLI Addon.
   */
  isLeanesCLIAddon() {
    return !!this.pkg.keywords && this.pkg.keywords.indexOf('leanes-addon') > -1;
  }
  ...
  configPath() {
    ...
    if (this.pkg['leanes-addon'] && this.pkg['leanes-addon']['configPath']) {
      configPath = this.pkg['leanes-addon']['configPath'];
    }
   ...
   */
  config(env) {
    const _env = env === undefined ? process.env.LEANES_ENV : env;
    const key = `${this.configPath()}|${_env}`;
  ...
  */
  ...
  resolveSync(file) {
    return resolve.sync(file, {
      basedir: process.env.LEANES_NODE_PATH || this.root,
    });
  }
  ...
  */
  ...
  /**
    ...
    like `leanes-qunit` and `leanes-mocha`.
   ...
   */
  generateTestFile() {
    let message = 'Please install an LeanES test framework addon or update your dependencies.';
  ...
  static closestSync(pathName, _ui, _cli) {
    ...
    if (!isLeanesCLIProject(pkg)) {
      logger.info('ignoring parent project since it is not an leanes-cli project');
      return Project.nullProject(_ui, _cli);
    }
  ...
  static getProjectRoot() {
    ...
    if (pkg && pkg.name === 'leanes-cli') {
      logger.info("getProjectRoot: named 'leanes-cli'. Will use cwd: %s", process.cwd());
      return process.cwd();
    }
...
function isLeanesCLIProject(pkg) {
  return (
    pkg &&
    ((pkg.dependencies && Object.keys(pkg.dependencies).indexOf('leanes-cli') !== -1) ||
      (pkg.devDependencies && Object.keys(pkg.devDependencies).indexOf('leanes-cli') !== -1))
  );
}
