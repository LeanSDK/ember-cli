...
const logger = require('heimdalljs-logger')('leanes-cli:command');
...
let Command = CoreObject.extend({
  ...
  init() {
     ...
     * @type Boolean
     */
    this.isWithinProject = this.project.isLeanesCLIProject();
  ...
  validateAndRun(args) {
    return new Promise((resolve, reject) => {
      ...
      if (this.works === 'outsideProject' && this.isWithinProject) {
        throw new SilentError(`You cannot use the ${chalk.green(this.name)} command inside an leanes-cli project.`);
      }

      if (this.works === 'insideProject') {
        if (!this.project.hasDependencies()) {
          if (!this.isWithinProject && !this.project.isLeanesCLIAddon()) {
            throw new SilentError(
              `You have to be inside an leanes-cli project to use the ${chalk.green(this.name)} command.`
            );
          }
    ...
    if (isValidAlias(alias, option.type)) {
      ...
      if (Array.isArray(alias)) {
        ...
        try {
          ...
          const logger = require('heimdalljs-logger')('leanes-cli/models/command');
          logger.error(e);
        }
      }
      throw new Error(
  ...
  */
        ...
        this.ui.writeLine(
          chalk.yellow(
            `The option '--${key}' is not registered with the '${this.name}' command. ` +
              `Run \`leanes ${this.name} --help\` for a list of supported options.`
          )
        );
      }
  ...
  /**
    ...
    Basic help looks like this:
        ...
        leanes generate <blueprint> <options...>
