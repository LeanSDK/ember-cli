...
const logger = heimdallLogger('leanes-cli:addon');
const treeCacheLogger = heimdallLogger('leanes-cli:addon:tree-cache');
const cacheKeyLogger = heimdallLogger('leanes-cli:addon:cache-key-for-tree');
...
let addonProto = {
  ...
  */
  ...
  /**
    ...
    `leanes-addon.paths` in its `package.json`.
  ...
  */
  ...
  _initDefaultBabelOptions() {
    ...
    let defaultleanesCLIBabelOptions = {
      compileModules: true,
    };
    let leanesCLIBabelConfigKey = this._leanesCLIBabelConfigKey();
    this.__originalOptions[leanesCLIBabelConfigKey] = this.options[leanesCLIBabelConfigKey] = defaultsDeep(
      this.options[leanesCLIBabelConfigKey],
      defaultleanesCLIBabelOptions
    );
  },
   ...
   *
   * Example: leanes-data depends on ember-cli-babel and wishes to have
   * additional control over transpilation this method helps.
   *
   * ```js
   * // leanes-data/index.js
   * treeForAddon(tree) {
   *   let babel = this.findOwnAddonByName('ember-cli-babel');
   *
   *   return babel.transpileTree(tree, {
   *     // customize the babel step (see: leanes-cli-addons readme for more details);
   *   });
   * }
   * ```
   *
   ...
   */
  hintingEnabled() {
    const isProduction = process.env.LEANES_ENV === 'production';
    const testsEnabledDefault = process.env.LEANES_CLI_TEST_COMMAND === 'true' || !isProduction;
    const explicitlyDisabled = this.app && this.app.options && this.app.options.hinting === false;
  ...
  */
  isDevelopingAddon() {
    if (process.env.LEANES_ADDON_ENV === 'development' && this.parent instanceof Project) {
      ...
      if (this.root === this.parent.root) {
        if (parentName !== this.name && !process.env.LEANES_CLI_IGNORE_ADDON_NAME_MISMATCH) {
          let pathToDisplay = process.cwd() === this.root ? process.cwd() : path.relative(process.cwd(), this.root);

          throw new SilentError(
            'leanes-cli: Your names in package.json and index.js should match. ' +
              `The addon in ${pathToDisplay} currently have '${parentName}' in package.json and '${this.name}' in index.js. ` +
              'Until leanes-cli v3.9, this error can be disabled by setting env variable LEANES_CLI_IGNORE_ADDON_NAME_MISMATCH to "true". ' +
  ...
  */
  ...
  _leanesCLIBabelConfigKey() {
    let leanesCLIBabelInstance = findAddonByName(this.addons, 'ember-cli-babel');
  ...
  */
  ...
  /**
    ...
    *Note:* Any options set in the consuming application will override the addon.
    ...
    @public
    @method included
    @param {LeanesApp|LeanesAddon} parent The parent object which included this addon
  ...
  */
  compileAddon(tree) {
    if (!this.options) {
      this._warn(
        `LeanES CLI addons manage their own module transpilation during the \`treeForAddon\` processing. ` +
    ...
    let leanesCLIBabelConfigKey = this._leanesCLIBabelConfigKey();
    if (!this.options[leanesCLIBabelConfigKey]) {
      this._warn(
        `LeanES CLI addons manage their own module transpilation during the \`treeForAddon\` processing. ` +
          `\`${this.name}\` (found at \`${this.root}\`) has overridden the \`this.options.${leanesCLIBabelConfigKey}\` ` +
          `options which conflicts with the addons ability to transpile its \`addon/\` files properly. ` +
          `Falling back to default babel configuration options.`
      );

      this.options[leanesCLIBabelConfigKey] = this.__originalOptions[leanesCLIBabelConfigKey];
    }
...
/**
  This hook allows you to make changes to the express server run by leanes-cli.
...
Addon.prototype[BUILD_BABEL_OPTIONS_FOR_PREPROCESSORS] = function () {
  ...
  if (leanesCLIBabelInstance) {
    version = require(path.join(leanesCLIBabelInstance.root, 'package')).version;
  }
