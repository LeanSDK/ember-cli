...
class Builder extends CoreObject {
  ...
  readBuildFile(path) {
    // Load the build file
    let buildFile = findBuildFile('leanes-cli-build.js', path);
    if (buildFile) {
      return buildFile({ project: this.project });
    }

    throw new SilentError('No leanes-cli-build.js found.');
  }
  ...
  setupBroccoliBuilder() {
    ...
    process.env.LEANES_ENV = process.env.LEANES_ENV || this.environment;
