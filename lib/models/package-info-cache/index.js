...
const logger = require('heimdalljs-logger')('leanes-cli:package-info-cache');
...
class PackageInfoCache {
    ...
    errorEntries.forEach((errorEntry) => {
      switch (errorEntry.type) {
        ...
        case Errors.ERROR_PACKAGE_JSON_PARSE:
          ...
          break;
        case Errors.ERROR_LEANES_ADDON_MAIN_MISSING:
          logger.info(
            `  specifies a missing leanes-addon 'main' file at relative path '${path.relative(
              rootPath,
              errorEntry.data
            )}'`
          );
  ...
  loadProject(projectInstance) {
    ...
    if (!pkgInfo.processed) {
      ...
      if (projectInstance.cli && projectInstance.cli.root) {
        logger.info('Reading package for "leanes-cli": %o', projectInstance.cli.root);
   ...
   *     given project.
   *   - If an 'leanes-build-cli.js' dynamically adds new projects to the cache,
  ...
  _readPackage(packageDir, pkg, isRoot) {
    ...
    // If we have an leanes-addon, check that the main exists and points
    // to a valid file.
    if (pkgInfo.isAddon()) {
      logger.info('%s is an addon', pkg.name);

      // Note: when we have both 'main' and leanes-addon:main, the latter takes precedence
      let main = (pkg['leanes-addon'] && pkg['leanes-addon'].main) || pkg['main'];
      ...
      if (mainRealPath) {
        ...
        pkgInfo.addError(Errors.ERROR_LEANES_ADDON_MAIN_MISSING, mainPath);
        this.valid = false;
      }
    }
    ...
    const leanesAddonInfo = pkg['leanes-addon'];

    // Set up packageInfos for any in-repo addons
    if (leanesAddonInfo) {
      const paths = leanesAddonInfo.paths;

      if (paths) {
        paths.forEach((p) => {
          const addonPath = path.join(realPath, p); // real path, though may not exist.
          logger.info('Adding in-repo-addon at %o', addonPath);
          const addonPkgInfo = this._readPackage(addonPath); // may have errors in the addon package.
    ...
    if (pkgInfo.mayHaveAddons) {
      ...
      const nodeModules = this._readNodeModulesList(path.join(realPath, 'node_modules'));
