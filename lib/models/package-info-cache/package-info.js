...
const logger = require('heimdalljs-logger')('leanes-cli:package-info-cache:package-info');
...
class PackageInfo {
  constructor(pkgObj, realPath, cache, isRoot = false) {
    this.pkg = pkgObj;
    this.pkg['leanes-addon'] = this.pkg['leanes-addon'] || {};
    ...
    // is okay and, if we're an leanes addon, that we have a valid 'main' file.
  ...
  discoverAddonAddons() {
    ...
    this.addPackages(
      ...
      (pkgInfo) => !pkgInfo.isAddon() || pkgInfo.name === 'leanes-cli'
    );
  ...
  discoverProjectAddons() {
    ...
    this.addPackages(addonPackageList, project.isLeanesCLIAddon() ? [this] : null);
  ...
  dumpInvalidAddonPackages(addonPackageList) {
    ...
    if (invalidPackages.length > 0) {
      ...
      if (process.env.LEANES_CLI_ERROR_ON_INVALID_ADDON) {
        throw msg;
      } else {
