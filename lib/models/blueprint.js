...
const logger = require('heimdalljs-logger')('leanes-cli:blueprint');
...
/**
  ...
  invokes `leanes generate controller foo` then `__name__` becomes
  `foo`. When the `--pod` flag is used, for example `leanes
  ...
  `leanes generate controller foo` then `__path__` becomes
  `controller`. When the `--pod` flag is used, for example
  `leanes generate controller foo --pod` then `__path__`
...
let Blueprint = CoreObject.extend({
  ...
  */
  ...
  /**
    ...
    ```sh
    leanes generate controller foo --type=array --dry-run isAdmin:true
    ```
  ...
  */
  isUpdate() {
    if (this.project && this.project.isLeanesCLIProject) {
      return this.project.isLeanesCLIProject();
    }
  },
  ...
  */
  _generateFileMapVariables(moduleName, locals, options) {
    ...
    let inAddon = this.project.isLeanesCLIAddon() || !!options.inRepoAddon;
    let inDummy = this.project.isLeanesCLIAddon() ? options.dummy : false;
