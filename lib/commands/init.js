...
module.exports = Command.extend({
  name: 'init',
  description: 'Reinitializes a new leanes-cli project in the current folder.',
  works: 'everywhere',
  ...
  _defaultBlueprint() {
    if (this.project.isLeanesCLIAddon()) {
      return 'addon';
    } else {
      return 'app';
    }
  ...
  async run(commandOptions, rawArgs) {
    ...
    if (!packageName) {
      let message =
        `The \`leanes ${this.name}\` command requires a ` +
        `package.json in current folder with name attribute or a specified name via arguments. ` +
        `For more details, use \`leanes help\`.`;
