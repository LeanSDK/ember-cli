...
module.exports = Command.extend({
  name: 'new',
  description: `Creates a new directory and runs ${chalk.green('leanes init')} in it.`,
  ...
  async run(commandOptions, rawArgs) {
    ...
    if (!projectName) {
      message = `The \`leanes ${this.name}\` command requires a name to be specified. For more details, use \`leanes help\`.`;
    ...
    if (projectName === '.') {
      ...
      message = `Trying to generate an ${blueprintName} structure in this directory? Use \`leanes init\` instead.`;
