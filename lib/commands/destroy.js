...
module.exports = Command.extend({
  ...
  run(commandOptions, rawArgs) {
    ...
    if (!blueprintName) {
      return Promise.reject(
        new SilentError(
          'The `leanes destroy` command requires a ' +
    ...
    if (!entityName) {
      return Promise.reject(
        new SilentError(
          'The `leanes destroy` command requires an ' +
  ...
  printDetailedHelp() {
    ...
    ui.writeLine('');
    ui.writeLine('  Run `leanes help generate` to view a list of available blueprints.');
  },
});
