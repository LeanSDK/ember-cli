...
function isExperimentEnabled(experimentName) {
  ...
  if (process.env.LEANES_CLI_ENABLE_ALL_EXPERIMENTS && deprecatedExperiments.includes(experimentName)) {
    return false;
  }

  if (process.env.LEANES_CLI_ENABLE_ALL_EXPERIMENTS) {
    return true;
  }

  let experimentEnvironmentVariable = `LEANES_CLI_${experimentName}`;
  ...
  if (deprecatedExperiments.includes(experimentName)) {
    ...
    if (!deprecationPreviouslyIssued && isSpecifiedByUser) {
      console.warn(
        chalk.yellow(`The ${experimentName} experiment in leanes-cli has been deprecated and will be removed.`)
      );
